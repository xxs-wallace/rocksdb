...
class VBlockTable : public TableReader {
 public:
  ...
  class BinarySearchIndexReader {
   public:
    ...
    // returned object.
    InternalIteratorBase<BlockIndexHandle>* NewIterator(
 ...
 protected:
  Rep* rep_;
  VTableOptions* config_;
  explicit VBlockTable(Rep* rep, VTableOptions* config)
      : rep_(rep), config_(config) {}
  // No copying allowed
  explicit VBlockTable(const TableReader&) = delete;
 ...
 private:
  ...
  //     kBlockCacheTier
  InternalIteratorBase<BlockIndexHandle>* NewIndexIterator(
  ...
  Status CreateIndexReader(const ReadOptions& ro,
                           ...
                           BlockCacheLookupContext* lookup_context,
                           std::unique_ptr<BinarySearchIndexReader>* index_reader);
  ...
  Status PrefetchIndexAndFilterBlocks(
      ...
      InternalIterator* meta_iter, VBlockTable* new_table,
      bool prefetch_all, const BlockBasedTableOptions& table_options,
      const int level, size_t file_size,
      BlockCacheLookupContext* lookup_context);
...
struct VBlockTable::VTableOptions {
  VTableOptions(const ImmutableOptions& _ioptions, const EnvOptions& _env_options,
...
// These are immutable.
struct VBlockTable::Rep {
  Rep(VTableOptions options, bool skip_filters)
      : config(options), filter_policy(skip_filters ? nullptr : options.table_options.filter_policy.get()),
        filter_type(FilterType::kNoFilter),
        index_type(BlockBasedTableOptions::IndexType::kBinarySearch)
 {}
  ~Rep() { status.PermitUncheckedError(); }

  VTableOptions config;
  ...
  uint32_t level_for_tracing() const { return config.level >= 0 ? config.level : UINT32_MAX; }
  ...
  void CreateFilePrefetchBuffer(
    ...
    fpb->reset(new FilePrefetchBuffer(
        readahead_params, !config.ioptions.allow_mmap_reads /* enable */,
        false /* track_min_offset */, config.ioptions.fs.get(), config.ioptions.clock,
        config.ioptions.stats, readaheadsize_cb, usage));
  }
  ...
  std::size_t ApproximateMemoryUsage() const {
    ...
    usage += malloc_usable_size(const_cast<VBlockTable::Rep*>(this));
#else
    usage += sizeof(*this);
