...
#include "table/virtual_table/virtual_block_table_reader.h"
...
#define INSTANTIATE_BLOCKLIKE_TEMPLATES(T)                                     \
  template Status VBlockTable::RetrieveBlock<T>(                           \
  ...
  template Status VBlockTable::MaybeReadBlockAndLoadToCache<T>(            \
      FilePrefetchBuffer * prefetch_buffer, const ReadOptions& ro,             \
  ...
  template Status VBlockTable::LookupAndPinBlocksInCache<T>(               \
      const ReadOptions& ro, const BlockIndexHandle& handle,                        \
...
INSTANTIATE_BLOCKLIKE_TEMPLATES(Block_kIndexToBlob);
...
VBlockTable::~VBlockTable() {
  auto ua = rep_->uncache_aggressiveness.LoadRelaxed();
  if (ua > 0 && config_->table_options.block_cache) {
    if (rep_->filter) {
    ...
    if (rep_->index_reader) {
      {
        ...
        std::unique_ptr<InternalIteratorBase<BlockIndexHandle>> iiter_unique_ptr;
...
void VBlockTable::UpdateCacheHitMetrics(BlockType block_type,
                                            GetContext* get_context,
                                            size_t usage) const {
  Statistics* const statistics = config_->ioptions.stats;

  PERF_COUNTER_ADD(block_cache_hit_count, 1);
  PERF_COUNTER_ADD(block_cache_read_byte, usage);
  PERF_COUNTER_BY_LEVEL_ADD(block_cache_hit_count, 1,
                            static_cast<uint32_t>(config_->level));
...
void VBlockTable::UpdateCacheMissMetrics(BlockType block_type,
...
void VBlockTable::UpdateCacheInsertionMetrics(
...
void VBlockTable::SetupBaseCacheKey(const TableProperties* properties,
...
CacheKey VBlockTable::GetCacheKey(const OffsetableCacheKey& base_cache_key,
                                      const BlockIndexHandle& handle) {
  // Minimum block size is 5 bytes; therefore we can trim off two lower bits
  // from offet.
  return base_cache_key.WithOffset(handle.block_id);
}

Status VBlockTable::Open(
    ...
    std::shared_ptr<BlockReader> block_reader,
    uint64_t file_size,
  ...
  const bool prefetch_all = true;
  const bool preload_all = true;

  if (!ioptions.allow_mmap_reads && !env_options.use_mmap_reads) {
    s = PrefetchTail(ro, file.get(), file_size, false,
                     nullptr, prefetch_all, preload_all,
  ...
  VTableOptions options(
  ioptions, env_options, table_options, internal_comparator,
      file_size, level, false, false);
  Rep* rep = new VBlockTable::Rep(options, false);
  ...
  // Read metaindex
  std::unique_ptr<VBlockTable> new_table(
      new VBlockTable(rep, &rep->config));
  ...
  VTableOptions* opt = &rep->config;
  rep->create_context = BlockCreateContext(
      &opt->table_options, &opt->ioptions, opt->ioptions.stats,
      blocks_definitely_zstd_compressed, block_protection_bytes_per_key,
      opt->internal_comparator.user_comparator(), opt->index_value_is_full,
      opt->index_has_first_key);
  ...
  if (expected_unique_id != kNullUniqueId64x2) {
    ...
    TEST_SYNC_POINT_CALLBACK("VBlockTable::Open::PassedVerifyUniqueId",
                             &actual_unique_id);
  } else {
    TEST_SYNC_POINT_CALLBACK("VBlockTable::Open::SkippedVerifyUniqueId",
                             nullptr);
  ...
  TEST_SYNC_POINT_CALLBACK(
      "VBlockTable::Open::ForceNullTablePrefixExtractor",
  ...
  if (force_null_table_prefix_extractor) {
    ...
    if (rep->table_properties) {
      ...
      if (!st.ok()) {
        ...
        ROCKS_LOG_ERROR(rep->config.ioptions.logger,
  ...
  rep->persistent_cache_options =
      PersistentCacheOptions(rep->config.table_options.persistent_cache,
                             rep->base_cache_key, rep->config.ioptions.stats);
  ...
  rep->config.verify_checksum_set_on_open = ro.verify_checksums;
  s = new_table->PrefetchIndexAndFilterBlocks(
      ro, prefetch_buffer.get(), metaindex_iter.get(), new_table.get(),
      prefetch_all, table_options, level, file_size, &lookup_context);
...
Status VBlockTable::PrefetchTail(
  ...
  std::pair<size_t*, size_t*> prefetch_off_len_pair = {&prefetch_off,
                                                       &prefetch_len};
  TEST_SYNC_POINT_CALLBACK("VBlockTable::Open::TailPrefetchLen",
...
Status VBlockTable::ReadPropertiesBlock(
  ...
  if (!s.ok()) {
    ROCKS_LOG_WARN(config_->ioptions.logger,
    ...
    if (s.ok()) {
      s = ReadTablePropertiesHelper(
          ...
          config_->ioptions, &table_properties, nullptr /* memory_allocator */);
    }
    ...
    if (!s.ok()) {
      ...
      if (!s.ok()) {
        ROCKS_LOG_WARN(
            config_->ioptions.logger,
            "Problem reading or processing seqno-to-time mapping: %s",
            s.ToString().c_str());
      }
      rep_->config.blocks_maybe_compressed =
          rep_->table_properties->compression_name !=
          CompressionTypeToString(kNoCompression);
    }
  } else {
    ROCKS_LOG_ERROR(rep_->config.ioptions.logger,
                    "Cannot find Properties block from file.");
  }

  // Read the table properties, if provided.
  if (rep_->table_properties) {
    rep_->config.whole_key_filtering &=
        IsFeatureSupported(*(rep_->table_properties),
                           BlockBasedTablePropertyNames::kWholeKeyFiltering,
                           config_->ioptions.logger);
    rep_->config.prefix_filtering &= IsFeatureSupported(
        *(rep_->table_properties),
        BlockBasedTablePropertyNames::kPrefixFiltering, config_->ioptions.logger);

    rep_->config.index_key_includes_seq =
        rep_->table_properties->index_key_is_user_key == 0;
    rep_->config.index_value_is_full =
    ...
    if (min_ts_pos != props.end()) {
      rep_->config.min_timestamp = Slice(min_ts_pos->second);
    }
    auto max_ts_pos = props.find("rocksdb.timestamp_max");
    if (max_ts_pos != props.end()) {
      rep_->config.max_timestamp = Slice(max_ts_pos->second);
    }

    rep_->config.index_has_first_key =
        rep_->index_type == BlockBasedTableOptions::kBinarySearchWithFirstKey;

    s = GetGlobalSequenceNumber(*(rep_->table_properties), largest_seqno,
                                &(rep_->config.global_seqno));
    if (!s.ok()) {
      ROCKS_LOG_ERROR(config_->ioptions.logger, "%s", s.ToString().c_str());
    }
  }
  return s;
}

Status VBlockTable::ReadRangeDelBlock(
...
Status VBlockTable::PrefetchIndexAndFilterBlocks(
    const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
    InternalIterator* meta_iter, VBlockTable* new_table, bool prefetch_all,
    const BlockBasedTableOptions& table_options, const int level,
    size_t file_size, BlockCacheLookupContext* lookup_context) {
  ...
  const bool maybe_flushed = false;
  ...
  std::unique_ptr<VBlockTable::BinarySearchIndexReader> index_reader;
  s = new_table->CreateIndexReader(ro, prefetch_buffer, meta_iter, use_cache,
                                   prefetch_index, false, lookup_context,
                                   &index_reader);
  if (!s.ok()) {
    return s;
  }
...
void VBlockTable::SetupForCompaction() {}

std::shared_ptr<const TableProperties> VBlockTable::GetTableProperties()
    const {
  return rep_->table_properties;
}

const SeqnoToTimeMapping& VBlockTable::GetSeqnoToTimeMapping() const {
  return rep_->seqno_to_time_mapping;
}

size_t VBlockTable::ApproximateMemoryUsage() const {
  size_t usage = 0;
  if (rep_) {
...
Status VBlockTable::ReadMetaIndexBlock(
    const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
  ...
  Status s = ReadAndParseBlockFromFile(
      ...
      rep_->footer.metaindex_handle(), &metaindex, rep_->config.ioptions,
      rep_->create_context, true /*maybe_compressed*/,
      UncompressionDict::GetEmptyDict(), rep_->persistent_cache_options,
      GetMemoryAllocator(rep_->config.table_options), false /* for_compaction */,
      false /* async_read */);
...
template <typename TBlocklike>
Cache::Priority VBlockTable::GetCachePriority() const {
...
template <typename TBlocklike>
WithBlocklikeCheck<Status, TBlocklike> VBlockTable::GetDataBlockFromCache(
  ...
  Status s;
  Statistics* statistics = config_->ioptions.statistics.get();
  ...
  if (block_cache) {
    ...
    auto cache_handle = block_cache.LookupFull(
        ...
        config_->ioptions.lowest_used_cache_tier);
...
template <typename TBlocklike>
WithBlocklikeCheck<Status, TBlocklike> VBlockTable::PutDataBlockToCache(
    const Slice& cache_key, BlockCacheInterface<TBlocklike> block_cache,
  ...
  const ImmutableOptions& ioptions = config_->ioptions;
  const uint32_t format_version = config_->table_options.format_version;
  assert(out_parsed_block);
  assert(out_parsed_block->IsEmpty());
  ...
  if (block_cache && block_holder->own_bytes()) {
    ...
    if (s.ok()) {
      ...
      UpdateCacheInsertionMetrics(TBlocklike::kBlockType, get_context, charge,
                                  s.IsOkOverwritten(), config_->ioptions.stats);
    } else {
...
// std::unique_ptr<FilterBlockReader> VBlockTable::CreateFilterBlockReader(
...
//     const Status s = RetrieveBlock(
//       prefetch_buffer, ro, rep->filter_handle,
//       UncompressionDict::GetEmptyDict(), &filter_block, nullptr,
//       lookup_context,
...
InternalIteratorBase<BlockIndexHandle>* VBlockTable::NewIndexIterator(
...
// TODO?
template <>
DataBlockIter* VBlockTable::InitBlockIterator<DataBlockIter>(
    const Rep* rep, Block* block, BlockType block_type,
    DataBlockIter* input_iter, bool block_contents_pinned) {
  return block->NewDataIterator(rep->config.internal_comparator.user_comparator(),
                                rep->get_global_seqno(block_type), input_iter,
                                rep->config.ioptions.stats, block_contents_pinned,
                                false);
}

// TODO?
template <>
IndexBlockIter* VBlockTable::InitBlockIterator<IndexBlockIter>(
    const Rep* rep, Block* block, BlockType block_type,
    IndexBlockIter* input_iter, bool block_contents_pinned) {
  return block->NewIndexIterator(
      rep->config.internal_comparator.user_comparator(),
      rep->get_global_seqno(block_type), input_iter, rep->config.ioptions.stats,
      /* total_order_seek */ true, rep->config.index_has_first_key,
      rep->config.index_key_includes_seq, rep->config.index_value_is_full,
      block_contents_pinned, rep->config.user_defined_timestamps_persisted);
}

// Right now only called for Data blocks.
template <typename TBlocklike>
Status VBlockTable::LookupAndPinBlocksInCache(
    const ReadOptions& ro, const BlockIndexHandle& handle,
    CachableEntry<TBlocklike>* out_parsed_block) const {
  BlockCacheInterface<TBlocklike> block_cache{
      config_->table_options.block_cache.get()};
  ...
  auto cache_handle =
      block_cache.LookupFull(key, &create_ctx, GetCachePriority<TBlocklike>(),
                             statistics, config_->ioptions.lowest_used_cache_tier);
...
VBlockTable::MaybeReadBlockAndLoadToCache(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockIndexHandle& handle, const UncompressionDict& uncompression_dict,
  ...
  if (block_cache) {
    ...
    if (out_parsed_block->GetValue() == nullptr &&
      ...
      if (!contents) {
        ...
        StopWatch sw(config_->ioptions.clock, statistics, histogram);
        ...
        BlockFetcher block_fetcher(
            ...
            &tmp_contents, config_->ioptions, do_uncompress, maybe_compressed,
            TBlocklike::kBlockType, uncompression_dict,
            rep_->persistent_cache_options,
            GetMemoryAllocator(config_->table_options),
            /*allocator=*/nullptr);
        ...
        if (s.ok()) {
          ...
          s = PutDataBlockToCache(
              ...
              GetMemoryAllocator(config_->table_options), get_context);
        }
      } else {
...
VBlockTable::SaveLookupContextOrTraceRecord(
...
void VBlockTable::FinishTraceRecord(
  ...
  BlockCacheTraceRecord access_record(
      config_->ioptions.clock->NowMicros(),
...
WithBlocklikeCheck<Status, TBlocklike> VBlockTable::RetrieveBlock(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
    CachableEntry<TBlocklike>* out_parsed_block, GetContext* get_context,
    BlockCacheLookupContext* lookup_context, bool for_compaction,
...

  {
    ...
    StopWatch sw(config_->ioptions.clock, config_->ioptions.stats, histogram);
    s = ReadAndParseBlockFromFile(
        rep_->file.get(), prefetch_buffer, rep_->footer, ro, handle, &block,
        config_->ioptions, rep_->create_context, maybe_compressed,
...
bool VBlockTable::PrefixRangeMayMatch(
...
bool VBlockTable::PrefixExtractorChanged(
...
Statistics* VBlockTable::GetStatistics() const {
  return config_->ioptions.stats;
}
bool VBlockTable::IsLastLevel() const {
  return config_->level == config_->ioptions.num_levels - 1;
}

bool VBlockTable::FullFilterKeyMayMatch(
  ...
  const Slice* const const_ikey_ptr = &internal_key;
  bool may_match = true;
  size_t ts_sz = config_->internal_comparator.user_comparator()->timestamp_size();
  Slice user_key_without_ts = StripTimestampFromUserKey(user_key, ts_sz);
  if (config_->whole_key_filtering) {
    may_match = filter->KeyMayMatch(user_key_without_ts, const_ikey_ptr,
                                    get_context, lookup_context, read_options);
    if (may_match) {
      RecordTick(config_->ioptions.stats, BLOOM_FILTER_FULL_POSITIVE);
      PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_full_positive, 1, rep_->level);
    } else {
      RecordTick(config_->ioptions.stats, BLOOM_FILTER_USEFUL);
    ...
    may_match = filter->PrefixMayMatch(
        ...
        get_context, lookup_context, read_options);
    RecordTick(config_->ioptions.stats, BLOOM_FILTER_PREFIX_CHECKED);
    if (may_match) {
      // Includes prefix stats
      PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_full_positive, 1, rep_->level);
    } else {
      RecordTick(config_->ioptions.stats, BLOOM_FILTER_PREFIX_USEFUL);
      // Includes prefix stats
      PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_useful, 1, rep_->level);
    }
  }
...
void VBlockTable::FullFilterKeysMayMatch(
  ...
  if (rep_->whole_key_filtering) {
    ...
    if (after_keys) {
      RecordTick(config_->ioptions.stats, BLOOM_FILTER_FULL_POSITIVE, after_keys);
      PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_full_positive, after_keys,
                                rep_->level);
    }
    uint64_t filtered_keys = before_keys - after_keys;
    if (filtered_keys) {
      RecordTick(config_->ioptions.stats, BLOOM_FILTER_USEFUL, filtered_keys);
    ...
    filter->PrefixesMayMatch(range, prefix_extractor, lookup_context,
                             read_options);
    RecordTick(config_->ioptions.stats, BLOOM_FILTER_PREFIX_CHECKED, before_keys);
    uint64_t after_keys = range->KeysLeft();
    if (after_keys) {
      // Includes prefix stats
      PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_full_positive, after_keys,
...
Status VBlockTable::Get(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters) {
  ...
  // }
  TEST_SYNC_POINT("VBlockTable::Get:BeforeFilterMatch");
  const bool may_match =
      FullFilterKeyMayMatch(filter, key, prefix_extractor, get_context,
                            &lookup_context, read_options);
  TEST_SYNC_POINT("VBlockTable::Get:AfterFilterMatch");
  if (may_match) {
    IndexBlockIter iiter_on_stack;
    ...
    if (matched && filter != nullptr) {
      if (rep_->whole_key_filtering) {
        RecordTick(config_->ioptions.stats, BLOOM_FILTER_FULL_TRUE_POSITIVE);
      } else {
        RecordTick(config_->ioptions.stats, BLOOM_FILTER_PREFIX_TRUE_POSITIVE);
      }
      // Includes prefix stats
...
Status VBlockTable::MultiGetFilter(const ReadOptions& read_options,
                                       const SliceTransform* prefix_extractor,
...
Status VBlockTable::Prefetch(const ReadOptions& read_options,
...
Status VBlockTable::VerifyChecksum(const ReadOptions& read_options,
...
Status VBlockTable::VerifyChecksumInBlocks(
  ...
  FilePrefetchBuffer prefetch_buffer(
      readahead_params, !config_->ioptions.allow_mmap_reads /* enable */);
  ...
  for (index_iter->SeekToFirst(); index_iter->Valid(); index_iter->Next()) {
    ...
    BlockFetcher block_fetcher(
        ...
        &contents, config_->ioptions, false /* decompress */,
...
BlockType VBlockTable::GetBlockTypeForMetaBlockByName(
...
Status VBlockTable::VerifyChecksumInMetaBlocks(
  ...
  for (index_iter->SeekToFirst(); index_iter->Valid(); index_iter->Next()) {
    ...
    if (meta_block_name == kPropertiesBlockName) {
      ...
      s = ReadTablePropertiesHelper(read_options, handle, rep_->file.get(),
                                    ...
                                    config_->ioptions, &table_properties,
                                    nullptr /* memory_allocator */);
    } else if (config_->verify_checksum_set_on_open &&
      ...
      s = BlockFetcher(
              ...
              read_options, handle, &contents, config_->ioptions,
...
bool VBlockTable::EraseFromCache(const BlockHandle& handle) const {
}

bool VBlockTable::TEST_BlockInCache(const BlockHandle& handle) const {
  assert(rep_ != nullptr);
...
bool VBlockTable::TEST_KeyInCache(const ReadOptions& options,
                                      const Slice& key) {
...
void VBlockTable::TEST_GetDataBlockHandle(const ReadOptions& options,
                                              const Slice& key,
                                              BlockHandle& handle) {
  std::unique_ptr<InternalIteratorBase<IndexValue>> iiter(NewIndexIterator(
...
//  5. index_type
Status VBlockTable::CreateIndexReader(
    const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
...
uint64_t VBlockTable::ApproximateDataOffsetOf(
...
uint64_t VBlockTable::GetApproximateDataSize() {
...
uint64_t VBlockTable::ApproximateOffsetOf(const ReadOptions& read_options,
                                              const Slice& key,
                                              TableReaderCaller caller) {
...
uint64_t VBlockTable::ApproximateSize(const ReadOptions& read_options,
...
bool VBlockTable::TEST_FilterBlockInCache() const {
  assert(rep_ != nullptr);
  return rep_->filter_type != Rep::FilterType::kNoFilter &&
         TEST_BlockInCache(rep_->filter_handle);
}

bool VBlockTable::TEST_IndexBlockInCache() const {
  assert(rep_ != nullptr);

  return TEST_BlockInCache(rep_->index_handle);
}

Status VBlockTable::GetKVPairsFromDataBlocks(
...
Status VBlockTable::DumpTable(WritableFile* out_file) {
...
Status VBlockTable::DumpIndexBlock(std::ostream& out_stream) {
...
Status VBlockTable::DumpDataBlocks(std::ostream& out_stream) {
  // TODO: plumb Env::IOActivity, Env::IOPriority
  const ReadOptions read_options;
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(read_options, /*need_upper_bound_check=*/false,
...
void VBlockTable::DumpKeyValue(const Slice& key, const Slice& value,
...
void VBlockTable::MarkObsolete(uint32_t uncache_aggressiveness) {
