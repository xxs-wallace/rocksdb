...
ExtentFileBuilder::ExtentFileBuilder(const TitanDBOptions& db_options,
...
void ExtentFileBuilder::FillBlockWithPad() {
...
void ExtentFileBuilder::WriteHeader() {
...
void ExtentFileBuilder::Add(const BlobRecord& record,
...
void ExtentFileBuilder::AddSmall(std::unique_ptr<BlobRecordContext> ctx) {
  cached_contexts_.emplace_back(std::move(ctx));
}

void ExtentFileBuilder::EnterUnbuffered(OutContexts* out_ctx) {
...
void ExtentFileBuilder::FlushSampleRecords(OutContexts* out_ctx) {
...
void ExtentFileBuilder::WriteEncoderData(BlobHandle* handle) {
...
void ExtentFileBuilder::WriteRawBlock(const Slice& block, BlockHandle* handle) {
...
void ExtentFileBuilder::WriteCompressionDictBlock(
...
Status ExtentFileBuilder::Finish(OutContexts* out_ctx) {
  if (!ok()) return status();
...
void ExtentFileBuilder::Abandon() {}

uint64_t ExtentFileBuilder::NumEntries() { return num_entries_; }
