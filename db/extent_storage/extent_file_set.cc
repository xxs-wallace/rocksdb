...
ExtentFileSet::ExtentFileSet(const TitanDBOptions& options, TitanStats* stats,
...
Status ExtentFileSet::OpenManifest(uint64_t file_number) {
  Status s;
...
Status ExtentFileSet::WriteSnapshot(log::Writer* log) {
  Status s;
...
struct ExtentFileSet::ManifestWriter {
  Status status;
  bool done;
  port::CondVar cv;
...
Status ExtentFileSet::LogAndApply(VersionEdit& edit) {
  mutex_->AssertHeld();
  TEST_SYNC_POINT("ExtentFileSet::LogAndApply::Begin");
  edit.SetNextFileNumber(next_file_number_.load());
  ...
  while (!writer.done && &writer != manifest_writers_.front()) {
    TEST_SYNC_POINT("ExtentFileSet::LogAndApply::Wait");
    writer.cv.Wait();
  }
  if (writer.done) {
...

  {
    ...
    if (s.ok()) {
      ImmutableDBOptions ioptions(db_options_);
      s = SyncTitanManifest(stats_, &ioptions, manifest_->file());
      TEST_SYNC_POINT("ExtentFileSet::LogAndApply::AfterSyncTitanManifest");
    }
    mutex_->Lock();
  }
...
void ExtentFileSet::AddColumnFamilies(
    const std::map<uint32_t, TitanCFOptions>& column_families,
    const std::string& cache_prefix) {
...
Status ExtentFileSet::DropColumnFamilies(
...
Status ExtentFileSet::MaybeDestroyColumnFamily(uint32_t cf_id) {
...
Status ExtentFileSet::DeleteBlobFilesInRanges(uint32_t cf_id,
                                            const RangePtr* ranges, size_t n,
                                            bool include_end,
                                            SequenceNumber obsolete_sequence) {
  mutex_->AssertHeld();
  TEST_SYNC_POINT("ExtentFileSet::DeleteBlobFilesInRanges");
  auto it = column_families_.find(cf_id);
  if (it != column_families_.end()) {
    VersionEdit edit;
...
void ExtentFileSet::GetObsoleteFiles(std::vector<std::string>* obsolete_files,
...
void ExtentFileSet::GetAllFiles(std::vector<std::string>* files,
